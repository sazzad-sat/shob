...
const TUI_SCHEMA_URL = "https://shob.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from shob.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const shob = await shobFiles(input)
  for (const file of shob) {
...
async function shobFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.SHOB_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("shob", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "shob")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "shob"))
  }
  if (Flag.SHOB_CONFIG) files.push(Flag.SHOB_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "shob"))

...
async function withoutWatcher<T>(fn: () => Promise<T>) {
  ...
  const prev = process.env.SHOB_EXPERIMENTAL_DISABLE_FILEWATCHER
  process.env.SHOB_EXPERIMENTAL_DISABLE_FILEWATCHER = "true"
  try {
    return await fn()
  } finally {
    if (prev === undefined) delete process.env.SHOB_EXPERIMENTAL_DISABLE_FILEWATCHER
    else process.env.SHOB_EXPERIMENTAL_DISABLE_FILEWATCHER = prev
  }
}

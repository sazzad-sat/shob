...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@shob-ai/sdk/v2"
...
export const RunCommand = cmd({
  ...
  handler: async (args) => {
    ...
    async function share(sdk: OpencodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.SHOB_AUTO_SHARE && !args.share) return
    ...
    if (args.attach) {
      const headers = (() => {
        ...
        const username = process.env.SHOB_SERVER_USERNAME ?? "shob"
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createOpencodeClient({ baseUrl: "http://shob.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})

export async function runCommand(args: unknown) {
  const handler = RunCommand.handler
  if (!handler) throw new Error("Run handler is unavailable")

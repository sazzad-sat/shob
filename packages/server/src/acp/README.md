...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for shob.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal shob sessions
...
```bash
SHOB_ENABLE_QUESTION_TOOL=1 shob acp
```
...
- **Terminal Integration**: Full terminal support via shob's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | shob acp
```
...
ACP sessions map cleanly to shob's internal session model:

...
export const UpgradeCommand = {
  ...
  describe: "upgrade shob to the latest or a specific version",
  builder: (yargs: Argv) => {
    return yargs
      .positional("target", {
  ...
  handler: async (args: { target?: string; method?: string }) => {
    ...
    if (method === "unknown") {
      prompts.log.error(`shob is installed to ${process.execPath} and may be managed by a package manager`)
    ...
    if (Installation.VERSION === target) {
      prompts.log.warn(`shob upgrade skipped: ${target} is already installed`)
      prompts.outro("Done")
      return
    }

...
describe("plugin.loader.shared", () => {
  ...
  test("publishes session.error when install fails", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(path.join(dir, "shob.json"), JSON.stringify({ plugin: ["broken-plugin@9.9.9"] }, null, 2))
      },
    })
  ...
  test("publishes session.error when plugin init throws", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "shob.json"), JSON.stringify({ plugin: [file] }, null, 2))
  ...
  test("publishes session.error when plugin import fails", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "shob.json"), JSON.stringify({ plugin: [missing] }, null, 2))
...
`,
  ...
  test("skips external plugins in pure mode", async () => {
    ...
    const pure = process.env.SHOB_PURE
    ...
    try {
      ...
      if (pure === undefined) {
        delete process.env.SHOB_PURE
      } else {
        process.env.SHOB_PURE = pure
      }
    }
  })
